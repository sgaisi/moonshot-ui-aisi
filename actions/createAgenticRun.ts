...
export async function createAgenticRun(
  ...
  const result = agenticRunSchema.safeParse({
  ...
  const response = await fetch(
    `${config.webAPI.hostURL}${config.webAPI.basePathAgentic}?type=${AgenticCollectionType.COOKBOOK}`,
    {
      method: 'POST',
      headers: {
  ...
  redirect(`/agentic/session/run?runner_id=${responseBody.id}`);
}
