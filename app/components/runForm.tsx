...
export type RunFormMode = 'agentic' | 'benchmark';
...
export interface RunFormConfig {
  mode: RunFormMode;
...
interface RunFormProps extends RunFormConfig {
  ...
  children?: (dynamicState: {
    ...
    isRunAll: boolean;
    isPending?: boolean;
  }) => React.ReactNode; // Expose dynamic state to children
}
...
function RunForm({
  ...
  const [percentageOfPrompts, setPercentageOfPrompts] =
    React.useState(defaultPercentage);
  const [showErrorModal, setShowErrorModal] = React.useState(false);
  const [isRunAll, setIsRunAll] = React.useState(defaultPercentage === 100);
  const [formState, wrappedFormAction] = useFormState(
    formAction,
    initialFormValues
  );

  // Type assertion for formState since useFormState returns unknown
  const typedFormState = formState as FormState;

  const prevPercentageValue = React.useRef(percentageOfPrompts);

  React.useEffect(() => {
    if (typedFormState.formStatus === 'error') {
      ...
      {showErrorModal && (
        <Modal
          ...
          <div className="flex flex-col gap-2 items-start">
            {typedFormState.formErrors && (
              <FormStateErrorList formErrors={typedFormState.formErrors} />
            )}
          </div>
        </Modal>
      )}
        ...
        <div className="flex flex-col w-[55%] ipad11Inch:w-[60%] ipadPro:w-[65%] gap-2">
          <form
            ...
            <TextInput
              ...
              value={name}
              error={typedFormState.formErrors?.run_name?.[0]}
              labelStyles={{
                fontSize: '1rem',
            ...
            <TextArea
              ...
              value={description}
              error={typedFormState.formErrors?.description?.[0]}
            ...
            <div className="relative flex flex-col">
              <div className="w-full flex flex-col">
                ...
                {/* Mode-specific prompt information */}
                {mode === 'agentic' ? (
              ...
              <div className="flex justify-left gap-2 mb-8">
                <p className="text-moonpurplelight">
                  ...
                  <span
                    ...
                    ({mode === 'agentic' ? '100% of prompts' : 'prompts'})
                  </span>
                </p>
            ...
            {children &&
              children({
                percentageOfPrompts,
                isRunAll,
                isPending: false, // Can be extended if needed
              })}

            {/* Default configuration for agentic mode */}
            {mode === 'agentic' && !children && (
              <div className="bg-moongray-900 p-4 rounded-lg mb-4">
                ...
                <p className="text-moongray-400 text-sm mb-3">
                  Agentic tests will be run with default parameters optimized
                  for autonomous AI behavior testing.
                </p>
